  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 antialiased">
      <div className="bg-white rounded-3xl w-full max-w-3xl overflow-hidden shadow-2xl border-4 border-sky-300 flex flex-col max-h-[92vh]">
        {/* Kid Runner Top Bar */}
        <div className="bg-gradient-to-r from-sky-400 via-brand-500 to-indigo-500 px-3.5 sm:px-5 py-3 sm:py-4 text-white flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
            <div className="p-1.5 sm:p-2 rounded-2xl bg-white/20 backdrop-blur-xs shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] sm:text-xs text-sky-100 font-medium truncate">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              className="p-2 sm:p-2.5 rounded-2xl bg-white/20 hover:bg-white/30 text-white transition-colors"
        ...
        <div className="p-3.5 sm:p-8 flex-1 overflow-y-auto flex flex-col justify-center bg-gradient-to-b from-sky-50/50 to-white">
          {!completed ? (
            renderContent()
          ) : (
            /* Joyful Star Celebration Screen */
            <div className="text-center space-y-4 sm:space-y-5 py-4 sm:py-6 animate-in zoom-in-95 duration-300">
              <div className="w-16 h-16 sm:w-24 sm:h-24 mx-auto rounded-full bg-amber-400 flex items-center justify-center text-white shadow-xl ring-4 sm:ring-8 ring-amber-100 animate-bounce">
                <Star className="w-10 h-10 sm:w-14 sm:h-14 fill-white text-white" />
              </div>
              ...
              {/* Stars Earned Banner */}
              <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl bg-amber-100 border-2 border-amber-300 text-amber-950 font-black text-sm sm:text-lg shadow-sm">
                <Star className="w-5 h-5 sm:w-6 sm:h-6 fill-amber-500 text-amber-600" />
                <span>+{completionData?.stars_awarded || activity.star_reward || 2} Stars Earned!</span>
              </div>

              {completionData?.badge_unlocked && (
                <div className="p-3 sm:p-4 rounded-2xl bg-purple-50 border-2 border-purple-200 text-purple-900 max-w-md mx-auto space-y-1">
                  <div className="flex items-center justify-center gap-1.5 font-extrabold text-xs sm:text-sm text-purple-700">
                    <Trophy className="w-4 h-4 text-purple-600" />
                    <span>NEW BADGE UNLOCKED!</span>
                  </div>
                  <p className="text-xs sm:text-sm font-bold">{completionData.badge_unlocked}</p>
                </div>
              )}

              <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3 w-full sm:w-auto">
                <button
                  onClick={() => setCompleted(false)}
                  className="px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs sm:text-sm transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={onClose}
                  className="px-5 sm:px-6 py-2.5 sm:py-3 rounded-2xl bg-brand-600 hover:bg-brand-700 text-white font-extrabold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2 hover:scale-105 active:scale-95"
                >
